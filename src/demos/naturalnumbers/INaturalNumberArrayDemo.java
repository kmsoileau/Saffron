...
public class INaturalNumberArrayDemo
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		for (int i = 0; i < data.length; i++)
			triples[i] = new NaturalNumberTriple(new NaturalNumber(data[i][0]), new NaturalNumber(data[i][1]),
					new NaturalNumber(data[i][2]));

		INaturalNumberArray array = new INaturalNumberArray(triples);

		IProblemMessage s = new Conjunction(new NaturalNumberFixer(triples), new NaturalNumberFixer(array.getY(), 0),
				array).findModel(Problem.defaultSolver());
