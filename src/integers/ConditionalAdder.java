...
{
	public ConditionalAdder(IInteger[] numbers, IBitString membership, IInteger conditionalSum) throws Exception
	{
		this(numbers, membership.toBooleanVariableArray(), conditionalSum);
	}

	public ConditionalAdder(IInteger[] numbers, IBooleanVariable[] membership, IInteger conditionalSum) throws Exception
	{
		...
		stagingArray[stagingIndex++] = new IntegerEqualizer(subTotal[0], subAnswer[0]);
		for (int i = 1; i < numbers.length; i++)
		{
			subAnswer[i] = new Integer();
			subTotal[i] = new Integer();
			stagingArray[stagingIndex++] = new IntegerAdder(subTotal[i - 1], subAnswer[i], subTotal[i]);
		}

		stagingArray[stagingIndex++] = new IntegerEqualizer(subTotal[numbers.length - 1], conditionalSum);
	...
	public ConditionalAdder(IIntegerList numbersArray, IBitString membership, IInteger conditionalSum) throws Exception
	{
		this(numbersArray.getIntegerArray(), membership.getBVArray(), conditionalSum);
	}

	public ConditionalAdder(IIntegerList numbersArray, IBooleanVariable[] membership, IInteger conditionalSum)
			throws Exception
	{
